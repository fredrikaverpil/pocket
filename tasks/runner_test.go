...
func TestNew_CustomTasks(t *testing.T) {
	customTask := pocket.NewTask(
		...
		func(_ *pocket.RunContext) error {
			return nil
		},
	)
	...
	cfg := pocket.Config{
		AutoRun: pocket.Serial(
			pocket.NewTask(
				"deploy",
				"deploy the app",
				func(_ *pocket.RunContext) error { return nil },
			),
			pocket.NewTask(
				"release",
				"create a release",
...
func TestAllTasks_ReturnsAllTasks(t *testing.T) {
	cfg := pocket.Config{
		AutoRun: pocket.Serial(
			golang.Tasks(),
			pocket.NewTask("custom", "custom task", func(_ *pocket.RunContext) error { return nil }),
		),
	}
...
func TestParallel_Execution(t *testing.T) {
	...
	task1 := pocket.NewTask("task1", "task 1", func(_ *pocket.RunContext) error {
		count.Add(1)
		return nil
	})
	task2 := pocket.NewTask("task2", "task 2", func(_ *pocket.RunContext) error {
		count.Add(1)
		return nil
	})
...
func TestTask_RunsOnlyOnce(t *testing.T) {
	runCount := 0
	task := pocket.NewTask("once", "run once", func(_ *pocket.RunContext) error {
		runCount++
		return nil
	})
...
func TestManualRun_TasksRegistered(t *testing.T) {
	manualTask := pocket.NewTask("deploy", "deploy task", func(_ *pocket.RunContext) error {
		return nil
	})
...
func TestAutoRunTaskNames_TracksAutoRunTasks(t *testing.T) {
	autoTask := pocket.NewTask("build", "build task", func(_ *pocket.RunContext) error {
		return nil
	})
	manualTask := pocket.NewTask("deploy", "deploy task", func(_ *pocket.RunContext) error {
		return nil
...
func TestDuplicateTask_SameInstance_Deduplicated(t *testing.T) {
	...
	task := pocket.NewTask("shared", "shared task", func(_ *pocket.RunContext) error {
		return nil
	})
...
func TestDuplicateTask_DifferentInstances_FirstWins(t *testing.T) {
	...
	task1 := pocket.NewTask("deploy", "deploy v1", func(_ *pocket.RunContext) error {
		return nil
	})
	task2 := pocket.NewTask("deploy", "deploy v2", func(_ *pocket.RunContext) error {
		return nil
	})
...
func TestManualRun_WithPathFilter(t *testing.T) {
	task := pocket.NewTask("benchmark", "run benchmarks", func(_ *pocket.RunContext) error {
		return nil
	})
