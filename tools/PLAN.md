...
pk.WithOptions(
  TheTask,
  pk.WithFlags(TheFlags{...}),
  pk.WithXXX(...),
)
```

- `WithOptions(r Runnable, opts ...Option)` Runnable — structural wrapper.
  Creates a pathFilter node that can control path scope, flag  
  overrides, dedup behavior, etc.
- `WithFlags(flags any)` Option — returns an `Option (a func(*pathFilter))`.
  It's a modifier for a pathFilter, not a Runnable itself. It can  
  only be used inside WithOptions.

I find naming confusing. But wrapping a task with "options" in a
`pk.WithOptions` (type `pk.Option`) makes some sense. Can the API UX be made
more easily grokable? Or is it as good as it gets?
